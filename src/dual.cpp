...
int counter_clockwise_walk(const int face_id, int u, int v, const int n, 
    ...
    while (primal[u].faces[pos] == -1) {
        // record u as a vertex of face we are walking the boundary of counter clockwise
        cur_face.vertices[face_size++] = u;
        // record the face id for vertex u, note the position is unique identified by 
        // v's position in u's adj list
...
void construct_planar_dual(Fullerene (&F), const int p) {
    ...
    // by this point, we have determined which vertices and in which face by face id
    // we now want to determine the adjacency between faces in the planar dual
...
#if DEBUG_DUAL
            cout << "Face " << setw(4) << f << " is adj to face " << setw(4)
                 << face_g << endl;
#endif
        }
    }
